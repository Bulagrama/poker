...
st.write("Sincronizzato al 100% con la struttura delle cartelle di GitHub.")

IMAGE_DIR = "immagini_poker"
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# 1. SELEZIONE DELLO SCENARIO PRINCIPALE
...
# Mappatura rigorosa basata sulle maiuscole del tuo GitHub
if scenario == "RFI":
    ...
    sottocartella_stile = stile.upper() # Diventa CONSERVATIVE o MODERATE
    
    st.write("### 3. Tua Posizione:")
    pos = st.radio("Posizione:", ["UTG", "MP", "CO", "BTN", "SB"], horizontal=True)
    nome_file_principale = f"{pos.lower()}.jpg" # I file sono minuscoli (utg.jpg)
...
# Questo ti mostra nel widget grigio il percorso esatto aggiornato
...
if nome_file_bluff:
    ...
    with col1:
        if os.path.exists(path_principale):
            ...
            st.error(f"File mancante in: `{IMAGE_DIR}/{cartella_scenario}/{sottocartella_stile}/{nome_file_principale}`")
    with col2:
        if os.path.exists(path_bluff):
            st.image(path_bluff, caption="3-Bet Bluff Range", width=460)
        else:
            st.error(f"File mancante in: `{IMAGE_DIR}/{cartella_scenario}/{sottocartella_stile}/{nome_file_bluff}`")
else:
    if os.path.exists(path_principale):
        st.image(path_principale, caption=f"Range: {scenario}", width=520)
    else:
        st.error(f"❌ Immagine non trovata! Assicurati che sia in: `{IMAGE_DIR}/{cartella_scenario}/{sottocartella_stile if sottocartella_stile else ''}/{nome_file_principale}`")
